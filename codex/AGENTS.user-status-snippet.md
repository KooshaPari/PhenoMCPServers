...
## 25. User Response Status and iMessage

The local operator can be reached through the configured iMessage helper:

```bash
agent-imessage status --json     # estimate whether Koosha is likely to reply soon
agent-imessage inbox --limit 20  # read recent configured-user messages
...
agent-imessage log-response 4 --source imessage
```

When a Codex agent is about to stop because it is waiting for the user, first
check `agent-imessage hook-decision --text "<last assistant message>"` when
available. If it returns `reprompt_wait`, say clearly that the agent is waiting
for the user response only when the answer is actually needed. If it returns
`reprompt_default_or_defer`, make a reasonable default decision, defer the
question into the final answer, or continue with the least risky path. Do not
block progress solely because the user may be away.

`status --json` blends manual overrides, Messages activity, macOS idle time,
frontmost app/process activity, media assertions, recent input/output actions,
external signals such as eye tracking, and local learning files derived from
observed response latencies. External trackers should write short-lived signals
with explicit scores and max ages; do not treat stale eye/process/action data as
current presence.

Webcam eye tracking is opt-in only. Agents should not start camera capture
directly. A separate local tracker may publish only derived state, such as
`agent-imessage signal eye_tracking --score 0.9 --state looking --max-age-seconds 5`,
without storing frames or images.

Persistent observers should publish through the local backend:
...
Menu-bar monitor/tray runs via LaunchAgent at:
`~/Library/LaunchAgents/com.phenotype.agent-user-status-tray.plist`
...
The native monitor is a Swift/AppKit UI with a click-through OS overlay dot and
a borderless pinned top-right panel. The dev cursor tracker is only a stand-in
for real eye data:
...
Treat eye tracking, webcam, process, and user-presence telemetry as highly
confidential derived data. The backend should receive only abstract states,
scores, short-lived screen zones, and ETA/confidence; never raw camera frames,
screenshots, face/eye images, landmarks, biometric embeddings, raw gaze streams,
or medical inference labels.
