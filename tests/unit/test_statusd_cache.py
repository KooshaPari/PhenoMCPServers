...
@pytest.mark.requirement("FR-AGENT_USER_STATUS-004")
def test_actions_endpoint_cache_reuses_results_until_ttl(monkeypatch, statusd_server: str) -> None:
    now = 20.0
    calls: list[list[str]] = []
    ...
    def fake_run_agent(args: list[str], timeout: int = 30) -> dict[str, object]:
        ...
        return {
            "ok": True,
            "returncode": 0,
            "data": {"actions": [{"kind": "key_press"}], "call_count": len(calls)},
            "stderr": "",
            "timeout": timeout,
        }
    ...
    first_status, first = get_json(statusd_server, "/actions")
    second_status, second = get_json(statusd_server, "/actions")

    assert first_status == 200
    assert second_status == 200
    assert first["data"] == {"actions": [{"kind": "key_press"}], "call_count": 1}
    assert second["data"] == {"actions": [{"kind": "key_press"}], "call_count": 1}
    assert calls == [["actions"]]

    now = 26.0
    refreshed_status, refreshed = get_json(statusd_server, "/actions")

    assert refreshed_status == 200
    assert refreshed["data"] == {"actions": [{"kind": "key_press"}], "call_count": 2}
    assert calls == [["actions"], ["actions"]]


@pytest.mark.requirement("FR-AGENT_USER_STATUS-004")
def test_action_post_invalidates_cached_command_results(monkeypatch, statusd_server: str) -> None:
    calls: list[list[str]] = []
    ...
    assert get_json(statusd_server, "/actions")[1]["data"] == {"call_count": 1}
    assert get_json(statusd_server, "/actions")[1]["data"] == {"call_count": 1}

    post_status, post_payload = post_json(
        statusd_server,
        "/action",
        {"direction": "input", "kind": "key_press", "state": "typing", "max_age_seconds": 30},
    )
    refreshed = get_json(statusd_server, "/actions")[1]
    ...
    assert calls == [
        ["actions"],
        ["action", "input", "key_press", "--max-age-seconds", "30", "--state", "typing"],
        ["actions"],
    ]
