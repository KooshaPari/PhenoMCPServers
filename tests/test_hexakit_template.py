...
def test_validate_catalog_script_passes(rendered_dir: Path) -> None:
    ...
    shutil.copy(
        rendered_dir / "skills" / "echo-skill" / "SKILL.md", skill_dir / "SKILL.md"
    )
    shutil.copy(
        rendered_dir / "skills" / "echo-skill" / "skill.yaml", skill_dir / "skill.yaml"
    )
    plugin_dir = out_repo / "plugins" / "echo-bundle"
    plugin_dir.mkdir(parents=True, exist_ok=True)
    shutil.copy(
        rendered_dir / "plugins" / "echo-bundle" / "plugin.yaml",
        plugin_dir / "plugin.yaml",
    )
    shutil.copy(
        rendered_dir / "plugins" / "echo-bundle" / "mcp.json", plugin_dir / "mcp.json"
    )
    agent_dir = out_repo / "agents" / "echo-lead"
    agent_dir.mkdir(parents=True, exist_ok=True)
    shutil.copy(
        rendered_dir / "agents" / "echo-lead" / "agent.yaml", agent_dir / "agent.yaml"
    )
    server_dir = out_repo / "servers" / "echo"
    server_dir.mkdir(parents=True, exist_ok=True)
    shutil.copy(
        rendered_dir / "servers" / "echo" / "echo_server.py",
        server_dir / "echo_server.py",
    )
    shutil.copy(
        rendered_dir / "servers" / "echo" / "pyproject.toml",
